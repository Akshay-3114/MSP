...
def songs(request, filter_by):
    if not request.user.is_authenticated:
        ...
        try:
            song_ids = []
            for album in Album.objects.filter(Q(user=request.user) | Q(is_private = False)):
                for song in album.song_set.all():
                    song_ids.append(song.pk)

            shared_albums = Shared_album.objects.filter(receiver = request.user)
            name = []
            for u in shared_albums:
                name.append(u.album_title)
            shared_songs_id =  Song.objects.filter(album__in = name).values_list('id', flat=True)
            song_ids.extend(shared_songs_id)
