...
def index(request):
    if not request.user.is_authenticated:
        ...
        followee = Friend.objects.filter(follower=request.user)  #This line retrieves all instances of the Friend model where the follower field matches the currently logged-in user (request.user). ie., to get the list of people who the logged in user is following
        friends_albums = []
        for follo in followee:
            x = Album.objects.filter(Q(user=follo.followee)).exclude(is_private=True) #collecitng albums where the user is a followee and the album is not private.
            friends_albums = chain(friends_albums,x) #concatenate x with friends_albums
        ...
        query = request.GET.get("q") # for search .. Get
        if query:
            public_albums_search = public_albums.filter(
                Q(album_title__icontains=query) |
                Q(artist__icontains=query) #It retrieves albums where either the album_title field or the artist field contains a case-insensitive match to the query value. 
            ).distinct()
...
def register(request):
    form = UserForm(request.POST or None)
    if form.is_valid():
        user = form.save(commit=False) #doesn't save it in db yet
...
def profile(request):
    ...
    #find shared_albums which are private and shared by the user
    list_of_ids = []
    ...
    if request.method == "POST":
        ...
        if server_password == request.POST['confirm_password']:
            try:
                ...
                user.set_password(server_password)
                user.save()
                request.session['first_name'] = user.first_name
                request.session['last_name'] = user.last_name
                request.session['password'] = user.password
